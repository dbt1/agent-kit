...
$script:BootstrapVersion = "2026-04-02-active-work-v1"
...
function Write-HostAwareMemory {
  ...
  $activeWorkDir = Join-Path $script:AgentKitHome "workitems/active/$ProjectId"
  $hostFile = Join-Path $hostsDir "$($script:HostId).md"
  $indexFile = Join-Path $indexDir "$($script:HostId).md"

  Ensure-Dir $hostsDir
  Ensure-Dir $indexDir
  Ensure-Dir $activeWorkDir
...
## Active Work
- Directory: $activeWorkDir
- Claim File For This Host: $(Join-Path $activeWorkDir "$($script:HostId).md")
- Template: $(Join-Path $script:AgentKitHome "templates/workitems/active-work.md")
...
function Main {
  ...
  Ensure-Dir (Join-Path $script:AgentKitHome "workitems/active")
  ...
  Write-StateFile -StateFile $stateFile -ProjectPath $project -Profile $profile -MemoryFile $memoryFile -ProjectId $projectId -ActiveWorkDir $activeWorkDir
